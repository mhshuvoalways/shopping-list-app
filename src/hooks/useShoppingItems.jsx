...
const useShoppingItems = () => { //custom hooks should be start with use Name
    // URL params
    ...
    const createShoppingItem = useStoreActions(actions => actions.shoppingItems.create)
    const changeSearchTerm = useStoreActions(actions => actions.suggestions.changeSearchTerm)
    const addSuggestion = useStoreActions(actions => actions.addSuggestion.add)
    ...
    const getSuggestions = () => {
        ...
        return Object.values(suggestions).reduce((acc, curr)=> {
            if(!isFound(curr.text)) {
                acc.push(curr)
            }
            return acc
        }, [])
    ...
    return {createItem, getSuggestions, getShoppingItems, bucket}
}
